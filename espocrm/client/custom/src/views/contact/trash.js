define('custom:views/contact/trash', ['view'], Dep => class extends Dep {
    template = 'custom:contact/trash';
    ...
    setup() {
        ...
        this.setPageTitle('Restore records');
    }
    ...
    async loadRecords() {
        try {
            ...
            if (body) body.innerHTML = '<tr><td colspan="4" class="nexa-restore-empty text-danger">Deleted records could not be loaded.</td></tr>';
        }
    }
    ...
    applyFilters() {
        ...
        this.filteredRecords = this.records.filter(record => {
            const text = `${record.name || ''} ${record.emailAddress || ''}`.toLowerCase();
            const date = (record.deletedAt || '').slice(0, 10);
            return (!search || text.includes(search)) &&
                (!from || date >= from) && (!to || date <= to) &&
                (!deletedBy || record.deletedById === deletedBy);
        });
    ...
    renderRecords() {
        ...
        if (!this.filteredRecords.length) {
            body.innerHTML = '<tr><td colspan="4" class="nexa-restore-empty">No deleted contacts match these filters.</td></tr>';
    ...
    recordRow(record) {
        ...
        checkbox.setAttribute('aria-label', `Select ${record.name || record.emailAddress || 'contact'}`);
        ...
        name.textContent = record.name || 'Unnamed contact';
        email.textContent = record.emailAddress || 'No email recorded';
    ...
    toggleAll(event) {
        this.selectedIds = event.currentTarget.checked
            ? new Set(this.filteredRecords.map(record => record.id))
            : new Set();
        this.renderRecords();
    }
    ...
    async actionRestore() {
        ...
        Espo.Ui.notify('Restoring records...');
        try {
            ...
            Espo.Ui.error('The selected records could not be restored.');
        }
    }
    ...
    actionPurge() {
        ...
        this.createView('purgeConfirmation', 'custom:views/contact/modals/permanent-delete-confirmation', {
            count: ids.length,
        }, view => {
            view.render();
    ...
    async purgeRecords(ids) {
        Espo.Ui.notify('Permanently deleting records...');

        try {
            const result = await Espo.Ajax.postRequest('Nexa/contact/trash/purge', {ids});
            Espo.Ui.success(`${result.count} ${result.count === 1 ? 'contact' : 'contacts'} permanently deleted.`);
            this.selectedIds.clear();
            await this.loadRecords();
            document.dispatchEvent(new CustomEvent('nexa:contact-trash-changed'));
        } catch (error) {
            Espo.Ui.notify(false);
            Espo.Ui.error('The selected records could not be permanently deleted.');
        }
    }
});
