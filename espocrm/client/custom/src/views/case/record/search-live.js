define('custom:views/case/record/search-live', ['views/record/search'], Dep => class extends Dep {
    setup() {
        super.setup();
    ...
    afterRender() {
        super.afterRender();
        this.element?.classList.add('nexa-case-live-search');
        this.$textFilter?.attr('autocomplete', 'off').attr('aria-label', 'Search Cases as you type');
    ...
    filterLoadedRows(value) {
        ...
        const rows = document.querySelectorAll('#main .nexa-case-scroll-list tbody tr.list-row');
        let visible = 0;
        rows.forEach(row => {
            const matches = !query || normalize(row.textContent).includes(query);
            row.classList.toggle('nexa-case-row-filtered', !matches);
            row.setAttribute('aria-hidden', String(!matches));
            if (matches) visible++;
        });
        if (this.liveSearchStatus) this.liveSearchStatus.textContent = `${visible} loaded Case${visible === 1 ? '' : 's'} match.`;
    }
    search() { window.clearTimeout(this.liveSearchTimer); return super.search(); }
    runLiveSearch() {
        ...
        this.controlResetButtonVisibility(); this.isSearchedWithAdvancedFilter = this.hasAdvancedFilter();
    }
    setState(loading) {
        if (this.liveSearchStatus) this.liveSearchStatus.textContent = loading ? 'Checking all Cases.' : `${this.collection.length} Cases shown.`;
    }
});
